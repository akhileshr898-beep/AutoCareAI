...
class Vehicle(db.Model):
    ...
    last_service_odometer = db.Column(db.Integer, nullable=True)
    avg_daily_km = db.Column(db.Integer, default=30)
    service_interval_km = db.Column(db.Integer, default=5000)
    ...
    insurance_premium = db.Column(db.Numeric(10, 2), nullable=True)
    puc_certificate_number = db.Column(db.String(100), nullable=True)
    puc_issue_date = db.Column(db.Date, nullable=True)
    ...
    service_records = db.relationship('ServiceRecord', back_populates='vehicle', cascade='all, delete-orphan')
    fuel_records = db.relationship('FuelRecord', back_populates='vehicle', cascade='all, delete-orphan')
    component_records = db.relationship('ComponentRecord', back_populates='vehicle', cascade='all, delete-orphan')
    documents = db.relationship('VehicleDocument', back_populates='vehicle', cascade='all, delete-orphan')
    reminders = db.relationship('Reminder', back_populates='vehicle', cascade='all, delete-orphan')
...
class ServiceRecord(db.Model):
    ...
    service_center = db.Column(db.String(150), nullable=True)
    work_done = db.Column(db.Text, nullable=True)
    engine_oil = db.Column(db.String(150), nullable=True)
    ...
    next_service_odometer = db.Column(db.Integer, nullable=True)
    created_at = db.Column(db.DateTime, default=utc_now)
    updated_at = db.Column(db.DateTime, default=utc_now, onupdate=utc_now)

    vehicle = db.relationship('Vehicle', back_populates='service_records')
    component_records = db.relationship('ComponentRecord', back_populates='service_record', cascade='all, delete-orphan')
...
class FuelRecord(db.Model):
    ...
    odometer = db.Column(db.Integer, nullable=False)
    litres = db.Column(db.Numeric(8, 2), nullable=False)
    total_amount = db.Column(db.Numeric(10, 2), nullable=False)
    price_per_litre = db.Column(db.Numeric(8, 2), nullable=True)
    fuel_station = db.Column(db.String(150), nullable=True)
    full_tank = db.Column(db.Boolean, default=True, nullable=False)
    notes = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=utc_now)
    updated_at = db.Column(db.DateTime, default=utc_now, onupdate=utc_now)

    vehicle = db.relationship('Vehicle', back_populates='fuel_records')
...
class ComponentRecord(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    vehicle_id = db.Column(db.Integer, db.ForeignKey('vehicles.id'), nullable=False, index=True)
    service_record_id = db.Column(db.Integer, db.ForeignKey('service_records.id'), nullable=True, index=True)
    component_name = db.Column(db.String(100), nullable=False)
    action = db.Column(db.String(30), nullable=False, default="replaced")
    service_date = db.Column(db.Date, nullable=False)
    odometer = db.Column(db.Integer, nullable=False)
    expected_life_km = db.Column(db.Integer, nullable=False)
    notes = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=utc_now)

    vehicle = db.relationship('Vehicle', back_populates='component_records')
    service_record = db.relationship('ServiceRecord', back_populates='component_records')
